...
import {
    CODE_SUCCESS,
    CODE_TIME_OUT, ERR_AUTH_TOKEN_EXPIRED, ERR_AUTH_TOKEN_EXPIRED_CODE,
    ERROR_NETWORK_CODE,
...
export const handleErrorAxios = <T = Record<string, unknown>>(
  ...
  if (error.code === STATUS_TIME_OUT) {
    ...
    if (error.error_code === ERR_AUTH_TOKEN_EXPIRED) {
        // token expired
        return handleErrorApi(ERR_AUTH_TOKEN_EXPIRED_CODE) as unknown as ResponseBase<T>;
    }
